...
class GeneratorResNet(nn.Module):
    def __init__(self, img_shape=(3, 128, 128), res_blocks=9, c_dim=5):
        ...
        curr_dim = 64
        for _ in range(2):
            model1 += [nn.Conv2d(curr_dim, curr_dim * 2, 4, stride=2, padding=1, bias=False),
    ...
    def forward(self, img1, img2, img3, img4, img5, c):
        ...
        x = torch.cat([self.downmodel(x1), self.downmodel(x2), self.downmodel(x3), self.downmodel(x4), self.downmodel(x5)])
